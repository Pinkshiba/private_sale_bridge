...
contract TokenPresaleBridge is Ownable {
    ...
    mapping (address => bool) walletWhitelisted;
    mapping (address => uint256) purchasedAmount;
    event TokensBought(uint256 amount, address indexed sender);
    uint256 tokensPerBnb = 1666666; // 1.66M tokens per BNB (don't add 1e18 here)
    uint256 maxBnbAmount = 3*1e18;
    uint256 minBnbAmount = 5*1e17;
    bool public isInitialized = false;
    address public immutable tokenAddress;
    
    constructor() {
        address token = address(0); // PSHIB Token (update before deploying)
